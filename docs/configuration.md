...
| `ECHOSUPPORT_DEMO_MARKETING_SEED` | No          | `false`                 | Opt-in marketing copy for the seeded demo agent |
...
Use `OPENROUTER_BASE_URL` for OpenRouter or another OpenAI-compatible endpoint. For local models such as Ollama or vLLM, see [Local Models](local-models.md).
...
Set `ECHOSUPPORT_DEMO_MARKETING_SEED=true` only for a public marketing demo. It updates
the seeded `Demo Agent` with EchoSupport-specific demo copy and an allowed origin derived
from `PUBLIC_BASE_URL`. Leave it `false` for normal installs and production tenants.

Set `APP_EDITION=lite` before building the Docker image when you want the lightweight admin
experience for a knowledge-base assistant. Lite keeps the agent Profile, API keys, Knowledge
base, and Embed tabs visible. It hides Inbox, Appointments, Specialists, Services, CSAT,
