...
export function buildMessages(opts: BuildMessagesOptions): ChatMessage[] {
  ...
  const systemContent = [
    ...
    'If the visitor explicitly names a different service, treat it as a new booking request: discard any incompatible previous specialist, slot, and group-participant context. For a service question without a date, briefly confirm availability and ask which date to check; do not list slots or give a long service description unless asked.',
