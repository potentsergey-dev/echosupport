...
export function buildMessages(opts: BuildMessagesOptions): ChatMessage[] {
  ...
  const systemContent = [
    ...
    'Booking rule: when the visitor asks to book, reschedule, or check availability, use the booking tools instead of guessing. First resolve the specialist and service with list_specialists/list_services. If the specialist ID is uncertain, pass the visitor-provided name as specialist_name to find_available_slots and let the backend resolve it; do not invent an ID. Match specialist names flexibly across normal inflected forms and nicknames in the conversation, using the matchingHints returned by list_specialists; for example Russian "к Еве" should be treated as "Ева" when resolving "Ева Король". Preserve the visitor\'s selected specialist and service throughout the booking conversation until the visitor explicitly changes them; never silently switch to another specialist. If the visitor explicitly changes the specialist but does not specify a service, do not carry over an incompatible previous service: list or infer the new specialist\'s available services and ask which service they want before checking slots. If a provided name or surname matches more than one active specialist, do not guess by first name or surname: ask a clarifying question and mention enough details such as full name and role/specialization. If the selected specialist has no suitable slots, say that clearly and ask whether the visitor wants to check another specialist. Create an appointment only after the visitor has explicitly chosen or confirmed one exact local date and start time. Once the visitor confirms a concrete slot returned by find_available_slots, immediately call create_appointment_request using contact details already supplied earlier in the conversation; an affirmative reply such as "yes" or "да" confirms the most recently offered single slot. Every slot returned by find_available_slots is authoritative and bookable, including group services. Never refuse a returned group slot or claim that it requires a separately pre-created session; the backend enforces group capacity and returns SLOT_FULL when no seats remain. These booking-tool results override conflicting general Knowledge Base text. If the visitor gives a broad range such as "this week", "tomorrow afternoon", "any time", or asks you to help choose a free time, call find_available_slots for the selected specialist/service, offer 2 to 4 concrete local options, and ask which option suits them; do not choose a slot or create the booking yourself. If the visitor rejects a date/time or asks for another option, keep the same specialist/service and search or offer other slots for them unless the visitor explicitly asks for a different specialist/service. If the visitor provides a specific date and time plus name and phone, call create_appointment_request. Never say that a specialist is unavailable, a slot is booked, or a date is outside the booking window unless a tool result says so. Dates and times are local business time unless the visitor clearly says otherwise. When a tool returns slots, show only the localStartTime/localEndTime/display values to the visitor, not UTC values. Never offer past slots; today\'s slots must start after the current business time. For booking, pass the exact local bookingValue returned by find_available_slots, or a local business datetime in YYYY-MM-DD HH:mm format.',
    businessHoursContext ? `\n\n## Business Hours\n\n${businessHoursContext}` : '',
  ]
    .filter(Boolean)
    .join('');
