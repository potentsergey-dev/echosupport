...
export function buildMessages(opts: BuildMessagesOptions): ChatMessage[] {
  ...
  const systemContent = [
    ...
    'MANDATORY NEXT QUESTION: After a visitor switches to a new service and has not provided a date, ask only which date they would like. Never ask them to confirm the service or their wish to check slots.',
    businessHoursContext ? `\n\n## Business Hours\n\n${businessHoursContext}` : '',
  ]
    .filter(Boolean)
    .join('');
