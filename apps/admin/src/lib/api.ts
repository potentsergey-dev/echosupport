...
export function listDemoAccessRequests(): Promise<DemoAccessRequest[]> {
  return request('/admin/demo-access-requests');
}

export function approveDemoAccessRequest(
  id: string,
  expiresInHours = 24,
): Promise<{
  email: string;
  ...
  return request(`/admin/demo-access-requests/${id}/approve`, {
    method: 'POST',
    body: JSON.stringify({ expiresInHours }),
  });
}

export function rejectDemoAccessRequest(id: string): Promise<void> {
  return request(`/admin/demo-access-requests/${id}/reject`, { method: 'POST' });
}

export function revokeDemoAccessRequest(id: string): Promise<void> {
  return request(`/admin/demo-access-requests/${id}/revoke`, { method: 'POST' });
}
